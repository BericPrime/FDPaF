...
def parse_discord_data(file_path: str) -> List[Dict[str, Any]]:
    ...
    message_blocks = re.split(r'\n\[(\d+/\d+/\d+ \d+:\d+ [AP]M)\]', content)[1:]  # Splitting keeps timestamps, so we handle them manually
    messages = []

    for i in range(0, len(message_blocks), 2):  # Every pair is timestamp and message block
        ...
        # Remove attachments, embeds, and reactions
...
def save_to_csv(messages: List[Dict[str, Any]], output_path: str) -> None:
    ...
    """
    fields = ['timestamp', 'username', 'channel', 'content', 'reactions']
