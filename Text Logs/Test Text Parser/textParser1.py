...
def save_to_json(messages, metadata, output_file):
    """
    Save the messages and metadata to a single JSON file
    
    Args:
        messages: List of message dictionaries
        metadata: Dictionary containing channel metadata
        output_file: Path to the output file
    """
    data = {
        'metadata': metadata,
...
def main():
    parser = argparse.ArgumentParser(description='Parse Discord chat data')
    ...
    if args.format == 'jsonl':
        ...
        save_to_json(messages, metadata, args.output)
